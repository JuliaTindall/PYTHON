...
This program will create biome4 data using dans input files.
...
def get_cube_avg(expt, fieldname, units, fieldname_req,century, varname):
    ...
    cube = iris.load_cube(FILESTART + expt + '/' + expt 
                          + '_monthly_mean_' + fieldname + '.nc')
...
def get_temp_precip():
    ...
    cube = get_cube_avg(EXPTNAME, 'temperatures', 'degC', 
    ...
    pi_cube = get_cube_avg(PI_EXPT, 'temperatures', 'degC', 
                           'monthly mean temperature','p','temp')
    ...
    # precipitation
    cube = get_cube_avg(EXPTNAME,'precip', 'mm',
                         'monthly total precipitation','p','precip')
    # convert to mm/month multiply by (30) * (60 * 60 *24)
    cube.data = cube.data *  30. * 60. * 60. * 24.
    allcubes_plio.append(cube)

    pi_cube = get_cube_avg(PI_EXPT,'precip', 'mm',
                           'monthly total precipitation','p','precip')
    pi_cube.data = pi_cube.data *  30. * 60. * 60. * 24.
...
def get_sunshine():
    ...
    # mPWP
    cube_mPWP = get_cube_avg(EXPTNAME, 'cloud',
                             'percent', 
    ...
    # PI
    cube_PI = get_cube_avg(PI_EXPT, 'cloud',
                           'percent', 
                           'mean monthly percent of possible sunshine','p',
                            None)
