...
def get_sunshine():
    ...
    if FOR_ORIG == 'Y':
        cube_mPWP.data = cube_mPWP.data * 10.0
    ...
    cube_PI.attributes["name"] = 'cld'
    if FOR_ORIG == 'Y':
        cube_PI.data = cube_PI.data * 10.0
...
def extract_region(cubelist):
    """
    apply a lsm to the cubes
    """
    lonmin_dict = {'Australia' : 130.0}
    lonmax_dict = {'Australia' : 140.0}
    latmin_dict = {'Australia' : -31.0}
    latmax_dict = {'Australia' : -25.0}

    region_cubelist = iris.cube.CubeList([])

    lonmin = lonmin_dict.get(REGION)
    lonmax = lonmax_dict.get(REGION)
    latmin = latmin_dict.get(REGION)
    latmax = latmax_dict.get(REGION)
    
    for cube in cubelist:
        print(lonmin, lonmax, latmin, latmax)
        print(cube)
        try: 
            print(cube.coord('lon'))
        except:
        ...
        print(cube.coord('lat').points)
        lon_constraint = iris.Constraint(lon = lambda cell:
                                         lonmin < cell < lonmax)
        lat_constraint = iris.Constraint(lat = lambda cell:
                                         latmin < cell < latmax)
        lon_slice = cube.extract(lon_constraint)
        print(lon_slice)
        cubereg = lon_slice.extract(lat_constraint)
        
        region_cubelist.append(cubereg)            

    return region_cubelist
...
def main():
    ...
    allcubes_plio = extract_region(allcubes_plio_land)
    ...
    allcubes_anom = extract_region(allcubes_land_anom)
...
TMIN_FROM_FILE = 'Y'  # this is if we want to include absolute tmin

OUTFILE = FILESTART_UM + EXPTNAME + '/biome4/inputdata_' + REGION 
