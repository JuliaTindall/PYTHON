...
def get_temp_precip():
    ...
    # precipitation
    cube = get_cube_avg(EXPTNAME,'TotalPrecipitationRateInMMPerDay', 'mm',
                         'monthly total precipitation','p','precip')
    # convert to mm/month multiply by (30)
    cube.data = cube.data *  30.
    allcubes_plio.append(cube)

    pi_cube = get_cube_avg(PI_EXPT,'TotalPrecipitationRateInMMPerDay', 'mm',
                           'monthly total precipitation','p','precip')
    pi_cube.data = pi_cube.data *  30.
