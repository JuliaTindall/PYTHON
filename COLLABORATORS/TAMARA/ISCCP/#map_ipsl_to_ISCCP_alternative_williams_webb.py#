...
this will attempt to map the IPSL data onto the ISCCP weather states.  This
will use the alternative methodology of Williams and Webb
"""
import numpy as np
import iris
...
def find_euclidian_distances(in_centroid, ISCCP_centroid):

    """
    in_centroid is shape 3
    ISCCP_centroid is 10,3
    ...
    for i in range(0,10):
        WS_centroid = ISCCP_centroid[i,:]
        anomaly = in_centroid - WS_centroid
        sum_sq_WS[i] = np.sum(np.square(anomaly))
       
    return sum_sq_WS
 
def get_ind_cloud_regieme(ipsl_centroid, ISCCP_centroids,hist):
    """
    we have the centroids (tcc,ctp,albedo) from ipsl
    we will calculate the euclidean distance from these centroids to
    each of the histograms in the ISCCP_entroids
    ...
    euclidian_distance_WS= find_euclidian_distances(ipsl_centroid,
                                                      ISCCP_centroids)
...
def get_centroids(cube,ipsl_ind):
    """
    gets the centroids for
    total cloud cover, cloud top pressure and cloud albedo.  Normalised to 0-1
    """
    ...
    # tau was converted to alpha using the conversion table within the
    # isccp simulator code and linearly interpolted to obtain the central
    #albedos

    # from code:
    # note we need to average in albedo units
    # albedocld = (tau ** 0.895_wp) / ((tau ** 0.895_wp) + 6.82_wp)
    # this gives cot_axis = [  0.635   2.41    6.465  16.005  41.495 135.36 ]
    # alb =[0.08896761 0.2436828  0.43796408 0.63688904 0.8044792  0.9222089 ]

  
    tcc = np.sum(cube.data)
    if ipsl_ind:
        ctp_axis = cube.coord('pressure2').points
        cot_axis = cube.coord('atmosphere_optical_thickness_due_to_cloud').points
    else:
        ctp_axis = cube.coord('cloud top pressure dimension').points
        cot_axis = cube.coord('cloud optical thickness dimension').points
       
        
    alb_axis = (cot_axis ** 0.895) / ((cot_axis **0.895)+6.82)
    cubedata = cube.data
    ctp_avg = 0
    alb_avg = 0
    for ctp in range(0,7):
        for cot in range(0,6):
            #print(ctp_axis[ctp],cot_axis[cot],np.around(cubedata[ctp,cot],2))
            ctp_avg = ctp_avg + (cubedata[ctp,cot] * ctp_axis[ctp])
            alb_avg = alb_avg + (cubedata[ctp,cot] * alb_axis[cot])
    ctp_avg=ctp_avg / (tcc * 1000.)
    alb_avg=alb_avg / tcc
...
def get_all_cloud_regiemes(ipsl_cube, ISCCP_CR_cube):
    ...
    """
    # find the centroids of each of the ISCCP cubes as per Williams and Webb
    ISCCP_centroids = np.zeros((10,3))  # the first dimension (10) is the
                                  # weather state number.
                                  # The second dimension will give the
                                  # TCC, CTP and albedo centroid
    #print(ISCCP_CR_cube)
    for WS in range(0,10):
        cube = ISCCP_CR_cube[:,:,WS]
        tcc, ctp, albedo = get_centroids(cube,False)
        ISCCP_centroids[WS,0]=tcc
        ISCCP_centroids[WS,1]=ctp
        ISCCP_centroids[WS,2]=albedo
    ...
    for time in range(ndays):
        ...
        #for lat in range(30,35):
            #print(lats[lat])
            for lon in range(0,len(lons)):
                print(lons[lon])
                hist = ipsl_cube[time, :, :, lat,lon]
                tcc, cpt, albedo = get_centroids(hist,True)
                hist_centroids = [tcc,cpt,albedo]
                #print(hist_centroids)

                cloud_amount = np.sum(hist.data)
                if np.isfinite(hist_centroids[0]):
                   cloud_regieme, newhist = get_ind_cloud_regieme(hist_centroids,ISCCP_centroids,hist)
        ...
        filename = ('/home/users/jctindall/cloud_regiemes/ISCCP_alternative/'+ preind_plio_ind  + '_' + np.str(daystart + time) + '.nc')
...
def plot_avg_cloud_regieme_Tselioudis():
    ...
    """
    filestart = '/home/users/jctindall/cloud_regiemes/ISCCP_alternative/' + preind_plio_ind
    fig = plt.figure(figsize=[12,12])
    plotno=0
    ...
    plt.tight_layout()
    plt.savefig('figures/isccp_Tse_alt_WS_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.eps')
    plt.savefig('figures/isccp_Tse_alt_WS_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.png')
    plt.savefig('figures/isccp_Tse_alt_WS_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.pdf')


def plot_avg_cloud_regieme():
    """
    plots the average of the histograms in each cloud regieme
    the histograms are from the isccp data
    ...
    plt.tight_layout()
    plt.savefig('figures/isccp_alt_WS_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.eps')
    plt.savefig('figures/isccp_alt_WS_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.png')
    plt.savefig('figures/isccp_alt_WS_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.pdf')
...
def plot_map_RFO_cloud_regieme():
   ...
   # plt.tight_layout()
    plt.savefig('figures/isccp_alt_WS_RFO_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.eps')
    plt.savefig('figures/isccp_alt_WS_RFO_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.png')
    plt.savefig('figures/isccp_alt_WS_RFO_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.pdf')
    iris.save(CR_cubelist,'figures/isccp_alt_WS_RFO_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.nc',netcdf_format="NETCDF3_CLASSIC")
...
def plot_map_RFO_cloud_regieme_Tselioudis():
   ...
   # plt.tight_layout()
    plt.savefig('figures/isccp_Tse_alt_WS_RFO_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.eps')
    plt.savefig('figures/isccp_Tse_alt_WS_RFO_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.png')
    plt.savefig('figures/isccp_Tse_alt_WS_RFO_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.pdf')
    iris.save(CR_cubelist,'figures/isccp_Tse_alt_WS_RFO_' + preind_plio_ind +'_' + str(daystart) + '_' + str(daystart + ndays) + '.nc',netcdf_format="NETCDF3_CLASSIC")
...
def plot_map_RFO_anomaly():
    ...
    pifile = 'figures/isccp_Tse_alt_WS_RFO_PREIND_' + str(daystart) + '_' + str(daystart + ndays) + '.nc'
    pliofile = 'figures/isccp_Tse_alt_WS_RFO_PLIO_' + str(daystart) + '_' + str(daystart + ndays) + '.nc'
...
#                             norm=colors.BoundaryNorm(boundaries=V,ncolors=16))
   ...
   # plt.tight_layout()
    plt.savefig('figures/isccp_alt_WS_RFO_anom_' + str(daystart) + '_' + str(daystart + ndays) + '.eps')
    plt.savefig('figures/isccp_alt_WS_RFO_anom_' + str(daystart) + '_' + str(daystart + ndays) + '.png')
    plt.savefig('figures/isccp_alt_WS_RFO_anom_' + str(daystart) + '_' + str(daystart + ndays) + '.pdf')
...
def create_netcdf_1year():
    ...
    #for fileno in range(0,2):
        ...
        ncube.coord('longitude').var_name = 'lon'
        ncube.coord('latitude').var_name = 'lat'
       # print(ncube.coord('longitude').metadata)
    ...
    fileout = ('/home/users/jctindall/cloud_regiemes/ISCCP_alternative/'+ preind_plio_ind  + '_year_' + str(YEARREQ) + '.nc')


    #iris.save(oneyearcube,fileout, netcdf_format = "NETCDF3_CLASSIC")
    iris.save(oneyearcube,fileout,netcdf_format="NETCDF4")
    print('saved cube')
...
ndays=365
daystart =0
YEARREQ=2075  # 2025 Pliocene, 2075 preind
