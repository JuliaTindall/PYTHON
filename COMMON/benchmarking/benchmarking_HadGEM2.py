...
#PURPOSE 
#
#  This program will create a t-test to see whether the HadGEM2 run performed
#  by richard rigby is significantly different to our one
#  Richards is xpupd.  He has run for two years
#  mine is xkvja: /nfs/hera1/earjcti/um/HadGEM_data/xkvja/netcdf - we have 
#                stored 30 years of pf and pd
#
#NOTES
#  what is a t-test
...
class main():
    ...
    def __init__(self, field):
        ...
        if EXPT_TYPE == 'S':
            ...
            self.meanfile = (startfname + EXPT + '/average/' + EXPT + 
                         ...
                         np.str(EXPT_STARTYEAR + EXPT_NYEARS) + '.nc')
        ...
        if CNTL_TYPE == 'B':
            ...
            self.meanbench = (startfname + CNTL + '/average/' + CNTL + 
                              '_' + field + '_' + SEASON +'_mean_' +
                               ...
                               np.str(CNTL_STARTYEAR + CNTL_NYEARS) + '.nc')
        ...
        self.outstart = (startout + EXPT + '/benchmarking/' + EXPT + 
                         ...
                         np.str(EXPT_STARTYEAR + EXPT_NYEARS) + 
                         '_' + CNTL + '_' + np.str(CNTL_STARTYEAR) + 
                         '_' +  np.str(CNTL_STARTYEAR + CNTL_NYEARS) + '_' +
                         field  + '_' + SEASON)
        self.field = field
    ...
    def main_benchmark_field(self):
        ...
        # you want to look for a two-sample t-test
        data1 = ((((cube_exptsd.data ** 2.0) * (EXPT_NYEARS-1.0))
                          + ((cube_cntlsd.data ** 2.0) * (CNTL_NYEARS-1.0)))
                     / (CNTL_NYEARS + EXPT_NYEARS -2.0))
        data2 = (np.sqrt(data1)) * (np.sqrt((1.0/EXPT_NYEARS) + (1.0/CNTL_NYEARS)))
...
MODELTYPE = 'n' # n=HadGEM, y=HadCM3, F=Famous
