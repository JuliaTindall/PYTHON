...
def correct_cice_plio(HadCM3_cice_cube,field,expt):
    """
    ...
    print('correcting pliocene')
    lsmcube = iris.load_cube('/nfs/hera1/earjcti/UM_ANCIL/HadGEM/PLIOMIP3/P4_qrparm.mask.nc','LAND MASK (No halo) (LAND=TRUE)')
    ...
    # northern hemisphere and antarctica.  For PI we fixed Antarctica
    # differently because it was smoother.  For Pliocene we fix antarctica
    # in the same way as the NH
    ...
    lats_nh=np.arange(112,142,1)
    lats_sh=np.arange(0,30,1)
    latsreq=np.concatenate((lats_nh,lats_sh))
        ...
        for i in range(1,len(longitudes)-1):
            for j in latsreq:
                if cice_data[t,0,j,i] < 0.1 and lsmdata[0,0,j,i]==0.0:
                    ...
                    if lsmdata[0,0,j-1,i]==1.0 and origcice[t,0,j+1,i] > 0.3:
                    ...
                    if lsmdata[0,0,j,i-1]==1.0 and origcice[t,0,j,i+1] > 0.3:
...
def correct_cice_pi(HadCM3_cice_cube,field,expt):
    """
    we have put the sea ice onto the HadGEM grid. However there are some
    corrections needed because the original field had sea ice over the land
    1  Check to see where the ocean points without sea ice are.  (Compare
...
if expt == 'xpsic':
    HCM3c_cice_cube = correct_cice_pi('file','CICE',expt)
if expt == 'xpsid':
    HCM3c_cice_cube = correct_cice_plio('file','CICE',expt)
