...
def get_data(jobid, startyear, endyear):
    ...
    filename = ('/uolstore/Research/a/hera1/earjcti/um/' + jobid +
                '/database_averages/' + 
                jobid + '_Annual_Average_#pf_' + FIELD + '_' + STARTYEAR + 
                '_' + ENDYEAR + '.nc')
...
if meandiff_ann.data < 1.0:
    boundaries = [-2.0,-1.5,-1.0,-0.5,-0.25,0.0, 0.25,0.5,1.0, 1.5,2.0]
...
plt.savefig('/uolstore/Research/a/hera1/earjcti/um/' + EXPT +  '/avgplots/ann_' + EXPT + '-' + CNTL + '_' + FIELD + '_' + STARTYEAR + '_' + ENDYEAR + '.eps')
plt.savefig('/uolstore/Research/a/hera1/earjcti/um/' + EXPT +  '/avgplots/ann_' + EXPT + '-' + CNTL + '_' + FIELD + '_' + STARTYEAR + '_' + ENDYEAR +'.png')
plt.close()
