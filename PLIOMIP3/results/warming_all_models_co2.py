"""
#NAME
#   warming_all_models_co2.py  
#PURPOSE 
#
#   This will show the mean / winter / summer warming for all models, but it
#   will be due to CO2 only.
#
...
#EXPTS=['xqbwn']
(PIcontrolTann, 
 PIcontrolTjja, PIcontrolTdjf)=get_globmean_temperature(CONTROL_PI)
(LPcontrolTann, 
 LPcontrolTjja, LPcontrolTdjf)=get_globmean_temperature(CONTROL_LP)
...
for expt in PI_EXPTS:
    ...
    mean_ann_warming.append(exptTann - PIcontrolTann)
    mean_jja_warming.append(exptTjja - PIcontrolTjja)
    mean_djf_warming.append(exptTdjf - PIcontrolTdjf)
    exptnames_used.append(EXPTNAMES.get(expt))

for expt in LP_EXPTS:
    (exptTann,exptTjja,exptTdjf) = get_globmean_temperature(expt)
    mean_ann_warming.append(exptTann - LPcontrolTann)
    mean_jja_warming.append(exptTjja - LPcontrolTjja)
    mean_djf_warming.append(exptTdjf - LPcontrolTdjf)
    exptnames_used.append(EXPTNAMES.get(expt))
...
# labels
ax.set_title('Warming relative to CO2=280ppmv')
ax.set_ylabel('degC')
...
plt.axvspan(-0.5,0.5,facecolor='green',alpha=0.2)
plt.axvspan(0.5,2.5,facecolor='pink',alpha=0.2)
plt.axvspan(2.5,5.5,facecolor='yellow',alpha=0.2)
plt.axvline(x=0.5,ymin=0.0,ymax=1.0,color='black')
...
plt.text(-0.25,textpos,'')
plt.text(0.75, textpos,'PI boundary conditions')
plt.text(2.75,textpos,'Late Pliocene boundary conditions')
...
if LAND_OCN_IND == 'Globe':
    plt.savefig('/nfs/see-fs-02_users/earjcti/PYTHON/PLOTS/PLIOMIP3/results/warming_all_models_co2.eps')
else:
    plt.savefig('/nfs/see-fs-02_users/earjcti/PYTHON/PLOTS/PLIOMIP3/results/warming_all_models_co2_' + LAND_OCN_IND + '.eps')
plt.show()
