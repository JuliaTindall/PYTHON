#NAME
#    AMOC_based_on_PJV ammended for calculating ocean heat transport
#PURPOSE 
#
...
def get_mask(filename):
    ...
    Atlantic_cube = cube.copy(data=Global_mask)
    #plt.subplot(2,2,1)
...
def get_params(filename):
    ...
    return (dx,thickness,lats, coslats, v_cube,Tcube)
...
def calc_OHT(V_cube,dx,dz,coslats,T_cube):
    """
    ...
    ztotalVT=np.sum(V_cube.data * T_cube.data,axis=2) * dx * coslats
    ...
    OHT_data=np.ma.zeros(np.shape(vdzT_cube.data))
    OHT_data[0,:]=0.0
    for k in range(0,20):
        OHT_data[k,:]=(vdzT_cube.data[k-1,:]+OHT_data[k-1,:]) 
    OHT_data.mask = np.where(vdzT_cube.data.mask == 1.0, 1.0, 0.0)
    OHT_cube=vdzT_cube.copy(data=OHT_data *density * cp/ 1.0E15)
    OHT_cube.long_name = 'ocean heat transport summed above'
    V_cube.data = np.ma.where(V_cube.data > 1.0E20,-99999., V_cube.data)
    T_cube.data = np.ma.where(T_cube.data > 1.0E20,-99999., T_cube.data)
    ...
    #plt.subplot(2,1,1)
    #vals=np.arange(-10.,11.0,1.0)
    #qplt.contourf(OHT_cube,levels=vals,extend='both',cmap='RdBu_r')
    #plt.title('OHT')

    #plt.subplot(1,1,1)
    #vals=np.arange(-10.,11.0,1.0)
    plt.plot(totOHT_cube.coord('latitude').points,totOHT_cube.data,)
    plt.xlim(-1.0,1.5)
    plt.title('OHT: '+expt)
...
expt = 'xqbwc'
filename = '/home/earjcti/um/'+expt+'/pg/'+expt+'o#pg000003999c1+.nc'
Atlantic_cube=get_mask(filename)

# get other parameters we need
(dx,dz,lats,coslats,v_cube,Tcube) = get_params(filename)
...
# calculate OHT
calc_OHT(V_atl,dx,dz,coslats,Tcube)
sys.exit(0)
