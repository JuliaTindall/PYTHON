...
def plotdata(plotdata,fileno,lat,dep,titlename,minval,maxval,valinc,V,uselog,cbarname,pcplot,ax):
    ...
    if pcplot =='pc':
        ...
        cs = ax.contourf(lats,depths,plotdata,V,extend='both',cmap='RdBu_r')
        contourvals=[-24,-20,-16,-12,-8,-4,0,4,8,12,16,20,24]
...
##################################################
def plot_avg_moc(expt_name,extra,yearstart,yearend,ending,AMOC_PMOC_IND):
    """
    ...
    for year in range(yearstart,yearend):
        if extra == '#':
            datasetname = '/uolstore/Research/a/hera1/earjcti/um/' + expt_name + '/pk2/' + expt_name + 'o#pk' + str(year).zfill(9) + ending + '+.nc'
            print(datasetname)
        else:
            if year >= 10:
        ...
        ndepth=len(depth)
        nlatf=len(lat_full)
        AMOC_full=f.variables[field.get(AMOC_PMOC_IND)][:]
    ...
    plt.tight_layout()
    fileout = ('/uolstore/Research/a/hera1/earjcti/um/' + expt_name + '/MOC/'+ AMOC_PMOC_IND +
               '_' + expt_name + extra + 
               str(int(yearstart)) + '_' + str(int(yearend)) + 
               '.eps')
...
# this function will difference two AMOCs
    ...
    fig = plt.figure()
    ax1 = plt.subplot(2,1,1)
    plotdata(AMOC_diff,-99,lat,depth,titlename,-2,2.5,0.5,0.0,'n','Sv','avg',ax1)
    plt.tight_layout()
...
lat=retdata[0]
depth=retdata[1]
xpsie_AMOC=retdata[2]

sys.exit(0)

#EXPTNAME = 'xpsid'
#retdata=plot_avg_moc(EXPTNAME,'#',2,1199,'c1',basin) # extra is '#' or letter
#lat=retdata[0]
#depth=retdata[1]
#xpsid_AMOC=retdata[2]
...
#amoc_diff('xpsig','xpsid',xpsie_AMOC,xpsid_AMOC,lat,depth,basin)
...
plot_all_moc('xqbwg','#',3900,4000,plotallyears)
figureno=figureno+1
