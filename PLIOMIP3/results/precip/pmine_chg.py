...
#PURPOSE 
#
#  This program will show the precipitation minus evaporation change in the last ??? years of each 
#  experiment
"""
...
def get_avg(jobid, startyear):
    ...
    longfield = {'temp' : 'TEMPERATURE AT 1.5M',
                 ...
                 'pmine' : 'Precipitation minus evaporation'
                     }
    ...
    # get template map
    cube = iris.load_cube(filestart + '/um/xqbwd/pcpd/' + 
    ...
    for year in range(startyear, startyear+NYEARS):
        ...
        evap=np.squeeze(f.variables['field184'][:])
        
        tempavg = np.mean(precip - evap,axis=0)

        alltemps = alltemps + tempavg
        count=count+1.

    alltemps = alltemps / count
    if field == 'pmine': # convert to mm/day'
    ...
    print('jjj',avgtempcube,'jjj')

    vals = np.arange(-1,1.1,0.1)

    region_cube = avgtempcube.intersection(
        latitude=(0, 40),
        longitude=(-50, 50))

    qplt.contourf(region_cube,levels=vals,extend='both',cmap='RdBu')
    qplt.contour(region_cube,levels=[0.0])
    plt.gca().coastlines()
    plt.show()
    sys.exit(0)
...
#        vals = np.arange(-8.0, 9.0, 1.0)
    ...
    plt.savefig(filestart + '/um/' + EXPT +  '/avgplots/' + EXPT + '-' + CNTL + '_' + field + str(EXPT_STARTYEAR) + '-' + str(EXPT_STARTYEAR+NYEARS) +  '.eps')
    plt.savefig(filestart + '/um/' + EXPT +  '/avgplots/' + EXPT + '-' + CNTL + '_' + field+ str(EXPT_STARTYEAR) + '-' + str(EXPT_STARTYEAR+NYEARS) + '.png')
    plt.close()
    ...
    if field == 'precip': # also plot percentage change
        ...
        print('about to write to file')
        plt.savefig(filestart + '/um/' + EXPT +  '/avgplots/' + EXPT + '-' + CNTL + '_' + field+ str(EXPT_STARTYEAR) + '-' + str(EXPT_STARTYEAR+NYEARS) + '_ratio.eps')
        plt.savefig(filestart + '/um/' + EXPT +  '/avgplots/' + EXPT + '-' + CNTL + '_' + field+ str(EXPT_STARTYEAR) + '-' + str(EXPT_STARTYEAR+NYEARS) + '_ratio.png')
