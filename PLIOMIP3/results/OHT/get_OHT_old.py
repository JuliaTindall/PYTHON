...
def calc_OHT(V_cube,dx,dz,coslats,T_cube,T_cube_origgrid,basin):
    ...
    #print(T_cube.shape)
    #sys.exit(0)
    T_mean = np.average(T_cube.data, axis=3,keepdims=True)
    T_mean_depth = np.average(T_mean,axis=1,weights=dz,keepdims=True)
...
firstfile = (filestart+expt+'/pg/'+expt+'o#pg00000'+ str(yearend)+'c1+.nc')
