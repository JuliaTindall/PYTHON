#NAME
#
#  Note this doesn't have a refernce temperature so is not very good
#  for calculating OTH in the Pacific and Atlantic, but it is fine globally
#
...
#  oceans
#
#  it will write all the OHT out to 11a file.
#
#
...
def calc_OHT(V_cube,dx,dz,coslats,T_cube,basin):
    """
...
firstfile = (filestart+expt+'/pg/'+expt+'o#pg00000'+
                str(yearstart).zfill(4)+'c1+.nc')
...
(dx,dz,lats,coslats,v_cube,Tcube) = get_params(firstfile)
...
for year in range(yearstart,yearend):
    ...
    # calculate OHT
    OHT_glob_cube = calc_OHT(V_glob,dx,dz,coslats,Tcube,'Global')
    OHT_Atl_cube = calc_OHT(V_atl,dx,dz,coslats,Tcube,'Atlantic')
    OHT_Pac_cube = calc_OHT(V_Pac,dx,dz,coslats,Tcube,'Pacific')
...
fileout = filestart + expt + '/OHT_' + expt + '_' + str(yearstart) + '_' + str(yearend) + '.nc'
print(fileout)
