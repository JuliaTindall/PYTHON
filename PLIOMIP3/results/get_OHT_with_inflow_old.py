...
#  oceans
#
#  For the atlantic and the Pacific there may be some inflow through the CAS
#  so account for this.
#
#  Southward transport is that flowing from north + that coming from the CAS
#
#  it will write all the OHT out to a file.
#
#  The heat transport equation is back to being
#  rho * cp * v * theta  (as we no longer need a reference temperaturet
#
#  used a reference temperature of 0degC (273.15K)
...
def get_mask():
    ...
    cube_full = iris.load_cube('/home/earjcti/um/'+expt+'/pg/'+
                                expt + 'o#pg000003999c1+.nc',
    ...
    lat_constraint = iris.Constraint(latitude=lambda cell: -10 <= cell <= 15)
    lon_constraint = iris.Constraint(longitude=lambda cell: 260 <= cell <= 300)

    Atlantic_cube_sub = Atlantic_cube.extract(lat_constraint & lon_constraint)


    #iris.save([Atlantic_cube,Atlantic_cube_sub],'temporary.nc',fill_value=-99999.)
...
def get_params(filename):
    ...
    u_cube = iris.load_cube(filename,'TOTAL OCEAN U-VELOCITY      CM S**-1')
    #v_cube = iris.util.squeeze(v_cube)
    v_cube.data = v_cube.data / 100.  # convert to m/2
    u_cube.data = u_cube.data / 100.  # convert to m/2
    ...
    return (dx,thickness,lats, coslats, v_cube,Tcube,Tcube_origgrid,u_cube)
...
def V_grid_mask(orig_cube,vgrid_cube):
    ...
    Atlantic_cube_sub = vmaskcube.extract(lat_constraint & lon_constraint)


    #iris.save([vmaskcube,Atlantic_cube_sub],'temporary2.nc',fill_value=-99999.)
    #sys.exit(0)
    
    return vmaskcube

def get_CAS_OHT(Vcube,Ucube,Tcube,mask_ugrid):
    """
    we want to find the ocean heat transport through the cas using the 
    same equation
    northward and eastward goes from pacific to atlantic
    """
    # these are the gridboxes we change to ocean in the CAS experiments
    # T-grid7 < lat < 13, 270 < lon < 285
    # lat=12.5, (13.125, 11.875)lon=273.75 (272.5,273.75, 275)
    # lat=10.0  (10.625, 9.375) lon=277.5   (276.25,277.5,278.75)
    # lat=7.5  (8.125, 6.875) lon=281.25 (280,281.25,282.5)
    #
    #  using the atlantic mask.  The change from Atlantic to pacific means that
    #  flow is northwards between 8.125 and 9.375 from lons 282.50-276.25
    #         and between         10.625 and 11.875 from lons 272.5-273.75
    #  flow is eastward between 271.25-272.5 from lats 11.875 and 13.125
    #         and between       275.0 - 276.25 from lats10.625 and 9.375
    #
    #  so vacross1=8.75 lons 282.5-276.25
    #     vacross2=11.25 lons 272.5-273.75
    #
    #  uacross1 = 271.88 lat=12.5
    #  uacross2 = 275.625 lats 11.25 and 10.0
    # orig
    #lonmin=276.0
    #lonmax=283.125
    #vacross=13.75
    ...
    #                print(lat,lon,mask_ugrid.data[0,j,i],mask_ugrid.data[0,j,i-1]#,
   #                       mask_ugrid.data[0,j+1,i],mask_ugrid.data[0,j-1,i])
   # sys.exit(0)
    ...
    northheat_flux = np.zeros(20) # calculate the amount of heat exchanged
    ...
    for j, lat in enumerate(lats):
        if lat in vacross:
            ix = vacross.index(lat)
            coslat = np.cos(lat * 2.0 * np.pi / 360.) 
            for i,lon in enumerate(lons):
                if lonmin[ix] < lon < lonmax[ix]:
                    for k,dep in enumerate(depths):
                        if not Vcube.data.mask[0,k,j,i]:
                            ...
                            northheat_flux[k] = northheat_flux[k] + flux_add

                            
    eastheat_flux=np.zeros(20) # this is U x T Xarea
    for i, lon in enumerate(lons):
        if lon in uacross:
            ix=uacross.index(lon)
            for j,lat in enumerate(lats):
                if latmin[ix] < lat < latmax[ix]:
                    for k,dep in enumerate(depths):
                        if not Ucube.data.mask[0,k,j,i]:
                            ...
                            eastheat_flux[k] = eastheat_flux[k] + flux_add

    northOHT=np.sum(northheat_flux) * density * cp / 1.0E15
    eastOHT=np.sum(eastheat_flux) * density * cp / 1.0E15

    print(northOHT,eastOHT,northOHT+eastOHT)
 
    return northOHT,eastOHT,vacross[0]

  


def calc_OHT(V_cube,dx,dz,coslats,T_cube,T_cube_origgrid,basin,U_cube,mask_vgrid):
    """
    ...
    if ((basin == 'Atlantic' or basin == 'Pacific') and 
        (expt == 'xpsie' or expt == 'xpsig' or
         expt == 'xqbwe' or expt == 'xqbwg')):
            (cas_North_OHT,
            cas_East_OHT,
            cas_lat)= get_CAS_OHT(V_cube,U_cube,T_cube,mask_vgrid)
            CAS_OHT = cas_North_OHT + cas_East_OHT
    else:
        CAS_OHT = 0.0
        cas_lat = -90.
    ...
    ztotalVT=np.sum(V_cube.data * (T_cube.data-273.15),axis=3) * dx * coslats
    ...
    # if atlantic or pacific remove CAS from total heat transport
    print(CAS_OHT)
    if CAS_OHT !=0.0:
        for i,lat in enumerate(totOHT_cube.coord('latitude').points):
            if lat < cas_lat:
                print(lat,totOHT_cube.data[0,i],CAS_OHT)
                
    
    sys.exit(0)

    #print(totOHT_cube.shape)
    print(totOHT_cube.coord('latitude').points[60])
    #print('0-10',totOHT_cube[0,0:10].data)
    #print('10-20',totOHT_cube[0,10:20].data)
    #print('20-30',totOHT_cube[0,20:30].data)
    #print('30-40',totOHT_cube[0,30:40].data)
    #print('40-50',totOHT_cube[0,40].data)
    #print('50-60',totOHT_cube[0,50:60].data)
    #print('90-100',totOHT_cube[0,90:100].data)
    #print(totOHT_cube[0,40].data)
    #print('vdzt',vdzT_cube[0,:,40].data * density * cp / 1.0E15)

    print('correct')
    print(totOHT_cube[0,60].data)
    print('vdzt',vdzT_cube[0,:,60].data * density * cp / 1.0E15)
...
#        totOHT_cube = totOHT_cube.extract(latitude_constraint)
    ...
    plt.plot(totOHT_cube.coord('latitude').points,totOHT_cube.data[0,:],)
    plt.xlim(-30.0,90.)
    plt.ylim(-4.0,4.0)
    plt.title('OHT: '+expt)
...
Atlantic_cube, Pacific_cube, Indian_cube, Global_cube =get_mask()
OHT_global_cubes = CubeList([])
OHT_atlantic_cubes = CubeList([])
OHT_pacific_cubes = CubeList([])


firstfile = ('/home/earjcti/um/'+expt+'/pg/'+expt+'o#pg00000'+
                str(yearstart)+'c1+.nc')
# get a v_cube for a v_grid
(dx,dz,lats,coslats,v_cube,Tcube,Tcube_origgrid,u_cube) = get_params(firstfile)
...
for year in range(yearstart,yearend):
    ...
    (dx,dz,lats,coslats,v_cube,Tcube,
     Tcube_origgrid,u_cube) = get_params(filename)
    ...
    U_atl = Atlmask_cube * u_cube # this is on grid 20 * 144 * 288
    U_glob = Globmask_cube * u_cube # 
    U_Pac = Pacmask_cube * u_cube # 
...
fileout = 'OHT_' + expt + '_' + str(yearstart) + '_' + str(yearend) + '.nc'
