...
def get_avg(jobid, startyear):
    ...
    for year in range(startyear, startyear+NYEARS):
        jncube = iris.load_cube('/nfs/hera1/earjcti/um/' + jobid + '/pcpd/' + 
                 jobid + 'a#pd' + str(year).zfill(9) + 'jn+.nc',field)
        jlcube = iris.load_cube('/nfs/hera1/earjcti/um/' + jobid + '/pcpd/' + 
                 jobid + 'a#pd' + str(year).zfill(9) + 'jl+.nc',field)
        agcube = iris.load_cube('/nfs/hera1/earjcti/um/' + jobid + '/pcpd/' + 
                 jobid + 'a#pd' + str(year).zfill(9) + 'ag+.nc',field)

        tempavg = np.squeeze((jncube.data + jlcube.data + agcube.data) / 3.0)
...
EXPTS = ['xqbwg']  # xpsic PI,  xpsij-lp490  xpsik - lp560
#EXPTS=['xqbwd']
EXPT_STARTYEAR = 3900
#EXPT = 'Eoi400_ARC4_2450-2499'

# data from good experiment
CNTL = 'xqbwe'  # xpsic pi, xpsid lp400
...
#        vals = np.arange(-8.0, 9.0, 1.0)
    ...
    plt.savefig('/nfs/hera1/earjcti/um/' + EXPT +  '/avgplots/jja_' + EXPT + '-' + CNTL + '_' + field + str(EXPT_STARTYEAR) + '-' + str(EXPT_STARTYEAR+NYEARS) +  '.eps')
    plt.savefig('/nfs/hera1/earjcti/um/' + EXPT +  '/avgplots/jja_' + EXPT + '-' + CNTL + '_' + field+ str(EXPT_STARTYEAR) + '-' + str(EXPT_STARTYEAR+NYEARS) + '.png')
    plt.close()
    ...
    if field == 'precip': # also plot percentage change
        ...
        print('about to write to file')
        plt.savefig('/nfs/hera1/earjcti/um/' + EXPT +  '/avgplots/jja_' + EXPT + '-' + CNTL + '_' + field+ str(EXPT_STARTYEAR) + '-' + str(EXPT_STARTYEAR+NYEARS) + '_ratio.eps')
        plt.savefig('/nfs/hera1/earjcti/um/' + EXPT +  '/avgplots/jja_' + EXPT + '-' + CNTL + '_' + field+ str(EXPT_STARTYEAR) + '-' + str(EXPT_STARTYEAR+NYEARS) + '_ratio.png')
        ...
        plt.title(titlename, fontsize=10)
        plt.gca().coastlines()
        plt.savefig('/nfs/hera1/earjcti/um/' + EXPT +  '/avgplots/jja_' + EXPT + '-' + CNTL + '_' + field+ str(EXPT_STARTYEAR) + '-' + str(EXPT_STARTYEAR+NYEARS) + '_NAmerica.eps')
        plt.savefig('/nfs/hera1/earjcti/um/' + EXPT +  '/avgplots/jja_' + EXPT + '-' + CNTL + '_' + field+ str(EXPT_STARTYEAR) + '-' + str(EXPT_STARTYEAR+NYEARS) + '_NAmerica.png')
        plt.close()
