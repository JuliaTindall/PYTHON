...
def global_enbal(exptname,yearstart,yearend):
    ...
    filestart=hera1+'/um/'+exptname+'/database_averages/'+exptname
...
def dh_zonal_enbal(exptname,yearstart,yearend):
   ...
   # outgoing lw  (toa)
    ...
    fileout=hera1 + '/um/'+exptname+'/avgplots/energybal/dh_temp_anom_'+exptname+'.eps' 
...
def rf_zonal_enbal(exptname,lsm):
  ...
  # outgoing sw (clear sky flux)
   ...
   # outgoing lw  (toa)
    ...
    print(lsm)
    if lsm=='pi':
        orog_fname='/uolstore/Research/a/hera2/apps/metadata/ancil/preind2/qrparm.orog.nc'
    if lsm=='lp':
        orog_fname=hera1+'/ancil/P4_enh/P4_enh_qrparm.orog.nc'
    if lsm=='ep':
        orog_fname='~earjcti/PYTHON/PROGRAMS/PLIOMIP3/make_hadcm3_bcs/Early_Pliocene/EP_orog.nc'
    ...
    print(lsm)
    if lsm=='pi':
        lsm_fname='/uolstore/Research/a/hera2/apps/metadata/ancil/preind2/qrparm.mask.nc'
    if lsm=='lp':
        lsm_fname=hera1+'/ancil/P4_enh/P4_enh_qrparm.mask.nc'
    if lsm=='ep':
        lsm_fname='~earjcti/PYTHON/PROGRAMS/PLIOMIP3/make_hadcm3_bcs/Early_Pliocene/EP_mask.nc'
...
def main_dh_zonal(preind_expt,plio_expt,yearstart,yearend):
    ...
    fileout=hera1+'/um/' + plio_expt + '/avgplots/energybal/DH_energybal'+plio_expt+'-'+preind_expt+'_' + str(yearstart) + '_' + str(yearend) + '.eps' 
    plt.savefig(fileout, bbox_inches='tight')  

    plt.close()

    filetext=hera1+'/um/' + plio_expt + '/avgplots/energybal/DH_energybal'+plio_expt+'-'+preind_expt+'_' + str(yearstart) + '_' + str(yearend) + '.tex' 
...
yearstart=1000
yearend=1200
hera1 = '/uolstore/Research/a/hera1/earjcti'
...
fileout=hera1+'/um/' + plio_expt + '/avgplots/energybal/RF_albedo_'+plio_expt+'-'+preind_expt+'_' + str(yearstart) + '_' + str(yearend) + '.eps' 
plt.savefig(fileout, bbox_inches='tight')  
...
fileout=hera1+'/um/' + plio_expt + '/avgplots/energybal/RF_energybal'+plio_expt+'-'+preind_expt+'_' + str(yearstart) + '_' + str(yearend) + '.eps' 
plt.savefig(fileout, bbox_inches='tight')  
fileout=hera1+'/um/' + plio_expt + '/avgplots/energybal/RF_energybal'+plio_expt+'-'+preind_expt+'_' + str(yearstart) + '_' + str(yearend) + '.png' 

plt.savefig(fileout, bbox_inches='tight')  

plt.close()

filetext=hera1+'/um/' + plio_expt + '/avgplots/energybal/RF_energybal'+plio_expt+'-'+preind_expt+'_' + str(yearstart) + '_' + str(yearend) + '.tex' 
