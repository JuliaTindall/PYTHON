...
if meanreq == 'y':
    ...
    for i,cube in enumerate(allexpts_cubes):
        ...
        if i ==1:
            ax2=ax.twinx()
            ax2.plot(latitudes,values-cntl_values,
                    label=period.get(exptnames[1]) +  ' - ' +
                     period.get(exptnames[0]),linewidth=2,color='grey',linestyle='dotted')
            anomvals = ((values - cntl_values) / values) * 100.
    ...
    #plt.show()
    #sys.exit()
    plt.savefig('OHT/'+basin+'OHT_mean_' + str(yearstart) + '_' + str(yearend) + '_anomaly.png')
    plt.close()
    sys.exit(0)
