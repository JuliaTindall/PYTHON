...
spec = gridspec.GridSpec(ncols=5, nrows=3,hspace=0.1,
                         ...
                         height_ratios=[0.1,1,0.2])


period = {'xqbwd':'LP','xqwbj':'LP$_{490}','xqbwe':'EP$_{400}$',
          'xqbwg':'EP','xqbwc':'PI'}
filestart = '/home/earjcti/um/'
fileend = '_Monthly_Average_#pd_Temperature_1400_1500.nc'

# read in data
file_LP = (filestart + 'xpsid/database_averages/xpsid' + fileend)
file_EP = (filestart + 'xpsig/database_averages/xpsig' + fileend)
file_EP400 = (filestart + 'xpsie/database_averages/xpsie' + fileend)
...
titlename = 'b) EP$_{400}$ - LP = ' + f'{EP400_LP_mean.data:.2f}' + '$^\circ$C'
...
titlename = 'c) EP - EP$_{400}$ = ' + f'{EP_EP400_mean.data:.2f}' + '$^\circ$C'
ax2.set_title(titlename,fontsize=13)
plt.gca().coastlines()


# now add colorbar
cax = fig.add_subplot(spec[1:3,1:4])
...
bottom = min(p.y0 for p in positions)- 0.05
...
#plt.show()
#sys.exit(0)
plt.savefig('Supp_forc_fact.png')
plt.close()
