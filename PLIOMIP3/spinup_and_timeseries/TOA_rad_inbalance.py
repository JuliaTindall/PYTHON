...
def plotdata(plotdata,fileno,lon,lat,titlename,minval,maxval,valinc,V,uselog,cbarname):
   ...
   # this is good for the globe
    map=plt.subplot(2,2,fileno+1,projection=ccrs.PlateCarree())
   # x, y = map(lons, lats)
    if V == 0:
        V=np.arange(minval,maxval,valinc)
    if uselog =='y':
        cs = map.contourf(lons,lats,plotdata,V,norm=mp.colors.PowerNorm(gamma=1./3.),extend="both")
        cbar = plt.colorbar(cs,orientation="horizontal")
    else:
        if uselog =='la':
            cs = map.contourf(lons,lats,plotdata,V,norm=mp.colors.SymLogNorm(linthresh=2.0,linscale=2.0,vmin=-32,vmax=32),cmap='RdBu_r',extend="both")
            cbar = plt.colorbar(cs,orientation="horizontal")

        else:
            if uselog =='a':
                cs = map.contourf(lons,lats,
                                  plotdata,V,cmap='RdBu_r',extend="both")
                cbar = plt.colorbar(cs,orientation="horizontal")
            else:
                print(np.shape(plotdata))
                cs = map.contourf(lons,lats,plotdata,V,extend="both")
                cbar = plt.colorbar(cs,orientation="horizontal",)
...
def annmean(switch,HadCM3,expt):
    ...
    # other things if the budsgets don't balance

    if HadCM3 == 'y':
        datasetname='/uolstore/Research/a/hera1/earjcti/um/'+expt+'/pcpd/'+expt+'a#pd00000' + century +'[7-9]????.nc'
        print(datasetname)
    ...
    # read in data from multiple files
        dirname='/uolstore/Research/a/hera1/earjcti/um/HadGEM_data/'+expt+'/netcdf/'
...
#average across the time dimension
    ...
    print('residual' + str(average_residual))
    titlename='residual, avg='+str(average_residual)+'(w/m2)'
    plotdata(residual,2,lon,lat,titlename,-100,100,20.0,0.0,'n',wm2)
    


    fileout='/uolstore/home/users/earjcti/PYTHON/PLOTS/PLIOMIP3/assess_spinup/TOA_rad/annmean_'+expt+'.eps' 
...
HadCM3='y'
exptname='xqbwe'
century='39'  # put century as 2 characters ie. 01 02 03  13
plt.figure(figureno)
