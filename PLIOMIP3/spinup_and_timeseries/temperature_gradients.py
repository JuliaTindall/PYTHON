...
#PURPOSE
#
#   This program will do a timeseries of the mean temperature southwards of 65S for
#   a) surface
#   b) top 100m
#   c) top 200m
#   d) top 1000m
#   e) 1000m-3000m
#
#  the temperature has previously been obtained and is in basin_diagnostics
#
#
#
# Julia 22/11/2025
#
# update Julia 30.1.2026:  we will just write out a timeseries of temperature
# on single levels
#
...
def get_temperature_band(year):
    """
    gets the temperature at each of the bands
    #   a) surface
    #   b) top 100m
    #   c) top 200m
    #   d) top 1000m
    ...
    # extract 3d temperature
    tempcube = cubes.extract('temperature basin')[0]
    # temperature southwards of 65S
    south_constraint = iris.Constraint(latitude=lambda v:v<latmax)
    temp_south_cube =tempcube.extract(south_constraint)

    # average over area
    lat = temp_south_cube.coord('latitude')
    lon = temp_south_cube.coord('longitude')
    depth = temp_south_cube.coord('depth_1')
    ...
    weights = iris.analysis.cartography.area_weights(temp_south_cube)

    # mean by depth
    temp_levels_cube = temp_south_cube.collapsed(['latitude','longitude'],
                                                 iris.analysis.MEAN,
                                                 weights=weights)

    surf_data = temp_levels_cube[0].data

    # extract cube by levels
    cube_100 = temp_levels_cube.extract(iris.Constraint(depth_1=lambda d:d<100))
    cube_200 = temp_levels_cube.extract(iris.Constraint(depth_1=lambda d:d<200))
    cube_1000 = temp_levels_cube.extract(
        iris.Constraint(depth_1=lambda d:d<1000))
    cube_1000_3000 = temp_levels_cube[13:16]
...
def get_temperature_level(year):
    """
    gets the temperature at each of the levels
    #  5m, 25m, 47.8m 203m, 447m, 666m, 995m, 1500m
    ...
    surf_data = temp_levels_cube[0].data
    #print(temp_levels_cube)
    #print(temp_levels_cube.coord('depth_1').points)
   
    # extract cube by levels
 
    data_5= (temp_levels_cube.extract(iris.Constraint(depth_1=5.0))).data
    data_25= (temp_levels_cube.extract(iris.Constraint(depth_1=25.0))).data
    data_47= (temp_levels_cube.extract(iris.Constraint(depth_1=47.85))).data
    data_203= (temp_levels_cube.extract(iris.Constraint(depth_1=203.7))).data
    data_447= (temp_levels_cube.extract(iris.Constraint(depth_1=447.05))).data
    data_666= (temp_levels_cube.extract(iris.Constraint(depth_1=666.3))).data
    data_995= (temp_levels_cube.extract(iris.Constraint(depth_1=995.55))).data
    data_1500=( temp_levels_cube.extract(iris.Constraint(depth_1=1500.85))).data
...
if single_levs == 'n': # we will get temperature bands
    all_surf = []
    all_100 = []
    all_200 = []
    all_1000 = []
    ...
    for year in range(startyear,endyear):
        surf_data, data_100,data_200,data_1000,data_1000_3000=get_temperature_band(year)
    ...
    fileout = (filestart+'/basin_diagnostics/'+exptname+'_Pacific_temperature'+
               str(startyear) +'_' + str(endyear) + '_'+str(latmax)+ '.txt')
 
    with open(fileout, "w") as f:
        string = ("Year, temp_surf,temp_0_100,temp_0_200," +
                  "temp_0_1000,temp_1000_3000\n") 
        f.write(string)
...
if single_levs == 'y': # we will get temperature on a range of levels
    #  5m, 25m, 47.8m 203m, 447m, 666m, 995m, 1500m
    ...
    for year in range(startyear,endyear):
        d5,d25,d47,d203,d447,d666,d995,d1500=get_temperature_level(year)
        all_years.append(year)
        all_5m.append(d5)
        all_25m.append(d25)
        all_47m.append(d47)
    ...
    fileout = (filestart+'/basin_diagnostics/'
               +exptname+'_Pacific_temperature_levs'+
               str(startyear) +'_' + str(endyear) + '_'+str(latmax)+ '.txt')
