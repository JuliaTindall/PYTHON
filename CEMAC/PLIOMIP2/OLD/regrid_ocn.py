...
def regrid_data(fieldnamein,fieldnameout,exptnamein,exptnameout,filename,modelname,linux_win,fielduse):
    ...
    if modelname=='HadCM3' or modelname=='MRI-CGCM2.3': # one field in a file 
        ...
        if modelname=='HadCM3' and fielduse=='SST':
               cube_temp.coord('unspecified').rename('surface')
               
        if modelname=='HadCM3' and fielduse=='NearSurfaceTemperature':
   ...
   # one field but multiple years in a file
    if ((modelname=='IPSLCM5A' or modelname=='IPSLCM5A2') and 
        (fieldnamein !='tos')):
    ...
    if modelname=='UofT':         
        ...
        regridded_cube.add_dim_coord(iris.coords.DimCoord(points, 
         ...
         # keep NorESM on original calandar and IPSLCM5A
         if (modelname!='NorESM1-F' and modelname!='NorESM-L' 
            and modelname!='IPSLCM5A' and modelname!='IPSLCM5A2'):
    ...
    # NORESM does not start at month=1, it starts at month=2.
    # we are doing dome roundabout way of reordering the data
    if modelname=='NorESM1-F' or modelname=='NorESM-L':
        allcubes=iris.cube.CubeList([])
        for mon in range(1,13):
            slice = mean_mon_data.extract(iris.Constraint(month=mon))
            # attempt to reorder time coordinat
            slice.coord('time').points=mon
            slice.coord('time').bounds=None
            allcubes.append(slice)
        mean_mon_data=(allcubes.merge_cube())
        
        
    mean_mon_data.long_name=fieldnameout
    iris.util.promote_aux_coord_to_dim_coord(mean_mon_data,'month')
    print(mean_mon_data)
...
filename=' '
linux_win='l'
modelname='HadCM3' # MIROC4m  COSMOS UofT EC-Earth3.1
                   # HadCM3 MRI-CGCM2.3
                   # new to this version: IPSLCM5A, IPSLCM5A2
                   #                      NorESM1-F NorESM-L
                   #                      IPSLCM6A
