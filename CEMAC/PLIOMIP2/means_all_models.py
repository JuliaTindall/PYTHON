...
def resort_coords(cube,levelno):
    ...
    here we will also equalise the units
    """
    if fieldnames[field] == 'SeaIceConc':
         cube.units = None
...
def plotmean(modelnames,field,exptname,cntlname,linux_win,units,individual_plot):
    ...
    iris.util.equalise_attributes(expt_cubes)
    iris.util.equalise_attributes(cntl_cubes)
    iris.util.equalise_attributes(anom_cubes)
    ...
    print(anom_cubes[0].metadata)
    print(anom_cubes[1].metadata)
#    sys.exit(0)
    ...
    iris.util.equalise_attributes(expt_month_cubes)
    iris.util.equalise_attributes(cntl_month_cubes)
...
#modelnames=['CESM2', 'COSMOS', 
#            'EC-Earth3.3', 'CESM1.2', 'HadCM3', 'CCSM4', 
#            'CCSM4-UoT', 'MRI2.3']
