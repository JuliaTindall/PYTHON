...
def main():
    """ 
    This program will estimate the climate sensitivity from the proxy data as follows:
    1.  read in proxy data from mcclymont
    2.  read in the file from the model which see's whether there is a significant relationship
        between Plio(Tanom) and ECS at each gridbox
    3.  For each proxy point
        a) check if there should be a significant relationship
    ...
    proxylat, proxylon, proxy_sst_anom, Npoints = obj.get_proxydata() 
    obj = GetPliovar('t1', 'UK37') # get data for t1 timeslice
    lats_UK37, lons_UK37, proxy_tanom_UK37, Npoints_UK37 = obj.get_proxydata() 
    ...
    for i in range(0,nproxies):
        ...
        print(proxylat[i], proxylon[i],grid_ss, pval[grid_ss])
...
FILEOUT = ('/nfs/hera1/earjcti/PLIOMIP2/proxydata/climate_sensitivity_Mcclymont_' + fileending.get(BAYSPLINE_BAYMAG_IND)+'.pdf')
FILEOUT_R = ('/nfs/hera1/earjcti/PLIOMIP2/proxydata/climate_sensitivity_redu_Mcclymont_' + fileending.get(BAYSPLINE_BAYMAG_IND)+'.pdf')
FILEOUT_S = ('/nfs/hera1/earjcti/PLIOMIP2/proxydata/climate_sensitivity_redu_significant_Mcclymont_' + fileending.get(BAYSPLINE_BAYMAG_IND)+'.pdf')
...
TEXTFILE = '/nfs/hera1/earjcti/regridded/alldata/data_like_figure9_mcclymont_' + fileending.get(BAYSPLINE_BAYMAG_IND)+'.txt'
