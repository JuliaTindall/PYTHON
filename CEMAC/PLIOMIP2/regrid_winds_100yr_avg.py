...
#Created on june 24 2020; copied from regrid_winds_50yr_avg.py
...
def regrid_data(exptnamein, filename, fielduse, constraint):
    ...
    # outfile
    if LINUX_WIN  == 'l':
        outstart = ('/nfs/hera1/earjcti/regridded100/'+MODELNAME+'/'+ EXPTNAMEOUT +'.'+
        FIELDNAMEOUT + '.')
        lsmstart = '/nfs/hera1/earjcti/regridded100/'
    else:
    ...
    cube = cube100.extract(constraint)
...
FILENAME  =  ' '
LINUX_WIN  =  'l'
MODELNAME  = "NorESM-L" # MIROC4m  COSMOS CCSM4UoT -EC-Earth3.3
                   # HadCM3 MRI
                   # IPSLCM5A,  IPSLCM5A2
                   # NorESM1-F NorESM-L
                   # IPSLCM6A GISS2.1G
