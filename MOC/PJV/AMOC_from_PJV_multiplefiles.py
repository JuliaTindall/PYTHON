#NAME
#    AMOC_from_PJV_multiplefiles
#PURPOSE 
#
#
#  I am hoping this will reproduce Pauls AMOC script
...
def get_mask(filename_nc):
    ...
    cube = iris.load_cube(filename_nc,'TEMPERATURE (OCEAN)  DEG.C')
    cube_data = cube.data
    ...
    return Atlantic_cube, Pacific_cube, Indian_cube
...
def get_params(filename):
    ...
    return (dx,thickness,lats, coslats, v_cube,depths2)
    ...
    for k in range(0,20):
        for j in range(0,nlatv):
            for i in range(0,nlonv):
                ...
                if (origdata[0,k,j,i] ==1 and origdata[0,k,j+1,i]==1 and
                    origdata[0,k,j,ip1]==1 and origdata[0,k,j+1,ip1]==1):
                    vmaskdata[0,k,j,i]=1.0
                else: #mask
                    vmaskdata.mask[0,k,j,i]=1.0
...
def calc_stream(V_cube,dx,dz,coslats,basin,depout):
    """
    ...
    phi_cube.long_name = 'Meridional Overturning Stream Function (' +basin + ')'
    phi_cube.var_name = 'Merid_' + basin
    phi_cube.attributes['title'] =   phi_cube.long_name
    phi_cube.attributes['name'] =   phi_cube.var_name
...
for year in range(startyear, endyear):
    filestart = '/uolstore/Research/a/hera1/earjcti/um/' + expt
    fileend = expt + 'o#pg00000' + str(year) + 'c1+.nc'
    filename = filestart + '/pg/' + fileend
    Atlantic_cube, Pacific_cube, Indian_cube = get_mask(filename)

    # get other parameters we need
    if year == startyear: 
        (dx,dz,lats,coslats,v_cube,depths) = get_params(filename)

    # put masks onto a V-grid
    Atlmask_cube = V_grid_mask(Atlantic_cube,v_cube)
    Pacmask_cube = V_grid_mask(Pacific_cube,v_cube)
    Indmask_cube = V_grid_mask(Indian_cube,v_cube)

    # show V in the Atlantic mask
    V_atl = Atlmask_cube * v_cube # this is on grid 20 * 144 * 288
    V_pac = Pacmask_cube * v_cube # this is on grid 20 * 144 * 288
    V_ind = Indmask_cube * v_cube # this is on grid 20 * 144 * 288
    V_glob = v_cube

    
    # calculate stream function for the basin
    Atl_stream = calc_stream(V_atl,dx,dz,coslats,'Atlantic',depths)
    Pac_stream = calc_stream(V_pac,dx,dz,coslats,'Pacific',depths)
