...
#    PLOT_d18o_fields
#PURPOSE
#    This program will plot d18o diagnostics in a standard stash file
# Julia 22/11/2016
...
def d18o_precip(filename):
    """
    plots d18o precip in filename
    """

    cube=iris.util.squeeze(iris.load_cube(filename,'Stash code = 338'))
    print(cube)
    tot_18o=cube[1,:,:]+cube[4,:,:]+cube[7,:,:]+cube[10,:,:]
    tot_16o=cube[0,:,:]+cube[3,:,:]+cube[6,:,:]+cube[9,:,:]

    d18o_cube = (tot_18o / tot_16o - 2005.2E-6)/ 2005.2E-9

    vals = np.arange(-80,20,5)
    qplt.contourf(d18o_cube,levels=vals,extend='both')
    plt.gca().coastlines()
    plt.show()

def d18o_runoff(filename):
    """
    plots d18o runoff in filename
    """

    cube_18o=iris.util.squeeze(iris.load_cube(filename,'Stash code = 209'))
    cube_16o=iris.util.squeeze(iris.load_cube(filename,'RIVER OUTFLOW INTO OCEAN  KG/M2/S  A'))
  
    d18o_data = (cube_18o.data / cube_16o.data - 2005.2E-6)/ 2005.2E-9
    d18o_data2 = np.where(cube_16o.data == 0.0, 0.0, d18o_data)
    d18o_cube=cube_18o.copy(data=d18o_data)
    d18o_cube2=cube_18o.copy(data=d18o_data2)


    for i, lon in enumerate(d18o_cube.coord('longitude').points):
        for j, lat in enumerate(d18o_cube.coord('latitude').points):
            if d18o_cube.data[j,i] > 100.:
                print(lon,lat,d18o_cube.data[j,i],cube_18o.data[j,i],cube_16o.data[j,i])


def d18o_fluxcorr(filename):
    """
    plots d18o runoff in filename
    """

    cube_18o=iris.util.squeeze(iris.load_cube(filename,'Stash code = 217'))
    cube_16o=iris.util.squeeze(iris.load_cube(filename,'P-E FLUX CORRECTION       KG/M2/S  A'))
    ...
    plt.subplot(2,2,1)
    vals = np.arange(-80,80,5)
    qplt.contourf(d18o_cube)
    plt.gca().coastlines()
    plt.show()
    sys.exit(0)
    ...
    plt.subplot(2,2,2)
    #vals = np.arange(-80,80,5)
    qplt.contourf(cube_18o)
    plt.gca().coastlines()

    plt.subplot(2,2,3)
    #vals = np.arange(-80,80,5)
    qplt.contourf(cube_16o)
    plt.gca().coastlines()

    plt.subplot(2,2,4)
    vals = np.arange(-80,80,5)
    qplt.contourf(d18o_cube2,levels=vals,extend='both')
    plt.gca().coastlines()
...
def d18o_ple(filename):
    """
    plots d18o runoff in filename
    """

    cube_18o=iris.util.squeeze(iris.load_cube(filename,'Stash code = 207'))
    cube_16o=iris.util.squeeze(iris.load_cube(filename,'PLE:PRECIP-EVAP INTO OCEAN KG/M2/S A'))
    ...
    plt.subplot(1,1,1)
    vals = np.arange(-80,80,5)
    qplt.contourf(d18o_cube,levels=vals)
    plt.gca().coastlines()
    plt.show()
    sys.exit(0)
...
filename = 'xqetca#pd000005742fb+.nc'
#d18o_precip(filename)

filename = 'xqetco#pf000005742fb+.nc'
#d18o_runoff(filename)

filename = 'xqetco#pf000005742fb+.nc'
#18o_fluxcorr(filename)
